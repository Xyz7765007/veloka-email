...
function richFields(
  ...
  return {
    ...
    Mode: intake.mode || "",
    "Email Count": intake.emails?.length ?? 0,
    Subjects: (intake.emails || [])
      .map((e, i) => `${i + 1}. ${e.subject || "(no subject)"}`)
      .join("\n"),
    "Overall Score": analysis.campaign?.overallScore ?? null,
    Grade: analysis.campaign?.grade || "",
    Headline: analysis.campaign?.headline || "",
    Verdict: analysis.campaign?.verdict || "",
    Recommendation: analysis.campaign?.recommendation || "",
    Winner: analysis.campaign?.winnerLabel || "",
    Model: model,
